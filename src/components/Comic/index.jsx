...
import layer_51 from './images/layer_51.png';
...
export default class Comic extends React.Component {
  ...
  initThree = () => {
    ...
    var layers = [layer_0, layer_1, layer_2, layer_3, layer_4, layer_5, layer_6, layer_7,layer_51];
    init();
    animate();
    function init() {
      ...
      for (let i=0; i<layers.length; i++) {
        let mesh = new THREE.Mesh(new THREE.PlaneGeometry(10.4, 16), new THREE.MeshPhysicalMaterial({
          map: new THREE.TextureLoader().load(layers[i]),
          transparent: true,
          side: THREE.DoubleSide
        }));
        mesh.position.set(0, 0, i);
        mesh.scale.set(2 - (i / aspect), 1 - (i / aspect), 1 - (i / aspect));
        layerGroup.add(mesh);
        // 文字
        if (i === 5) {
      ...
      Animations.animateCamera(camera, controls, { x: -3, y: 1, z: 20 }, { x: 0, y: 0, z: 0 }, 3600, () => { });
    }
    ...
    function animate() {
      ...
      // step += 0.01;
      // let step = 0
      animateLayer.position.x = 0 + Math.cos(2);
      animateLayer.position.y = 0 + Math.abs(Math.sin(1));
    }
  }
